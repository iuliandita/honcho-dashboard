...
const { workspaceId, peers, pinned = false }: Props = $props();
...
<Pane scrollable>
  <PaneHeader
    ...
    subtitle={`${t(settings.locale, pinned ? 'common.pinnedWorkspace' : 'common.workspace')} ${workspaceId}`}
  />
  <PaneList
    items={$query.data ?? []}
    ...
    hrefFor={(peer) => peerPath(peer.id, pinned ? undefined : workspaceId)}
  >
