...
export const keys = {
  ...
  session: (ws: string, peer: string, session: string) =>
    ['workspace', ws, 'peer', peer, 'session', session] as const,
  sessionMessages: (ws: string, peer: string, session: string) =>
    ['workspace', ws, 'peer', peer, 'session', session, 'messages'] as const,
  peerRepresentation: (ws: string, peer: string) =>
    ['workspace', ws, 'peer', peer, 'representation'] as const,
  peerProfile: (ws: string, peer: string) =>
    ['workspace', ws, 'peer', peer, 'profile'] as const,
